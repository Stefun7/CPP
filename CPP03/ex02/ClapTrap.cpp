...
/*   Updated: 2025/12/27 12:05:49 by stephen          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
ClapTrap::ClapTrap() : name("Omega"), hitPoints(10), energyPoints(10), attackDamage(0){
	std::cout << "Initiating Clap" << std::endl;
}
...
ClapTrap::ClapTrap(const ClapTrap &C) : name(C.name), hitPoints(C.hitPoints), energyPoints(C.energyPoints), attackDamage(C.attackDamage){
	std::cout << "Cloning Clap" << std::endl;
}

ClapTrap::ClapTrap(std::string name) : name(name), hitPoints(10), energyPoints(10), attackDamage(0){
	std::cout << "Assembling Clap" << std::endl;
}


ClapTrap::ClapTrap(std::string name, unsigned int hp, unsigned int enrg, unsigned int dmg) :
	name(name), hitPoints(hp), energyPoints(enrg), attackDamage(dmg){
	std::cout << "Initiating Clap" << std::endl;
}

ClapTrap::~ClapTrap(){
	std::cout << "Clap terminated" << std::endl;
}

ClapTrap &ClapTrap::operator=(const ClapTrap &C){
	std::cout << "Assigning Clap" << std::endl;
	if (this != &C)
	{
		this->name = C.name;
